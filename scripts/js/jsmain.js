	...
	$("#tipe_0").click(function(){
		...
		$("#sueldo_actual_d").click(function(){
			...
			$("#sect_medio_empleo_padre input[type=text]").click(function(){
				...
				$("#idioma_padre_tra input[type=text]").click(function(){
					...
					$("#sect_empresa_padre input[type=text]").click(function(){
						desmarcar_chekbox( "#sect_empresa_padre", "#sect_empresa_padre input[type=text]" );
					})

						$("#puesto_trabajo_padre input[type=text]").click(function(){
							desmarcar_chekbox( "#puesto_trabajo_padre", "#puesto_trabajo_padre input[type=text]" );
						})
		...
		$("#btn_aceptar").click(function(){
						...
						if( 
							...
							if( $("#"+id_select)[0].type != "checkbox" ){
								...
								if( do_not == "id_input_pos"){
									...
									console.log( id_extra_father, do_not, id_select );

								var input_tip = document.getElementById("posgrado_a");

								if( id_select == "tip_pos_0" ){
									input_tip.value = "***************";
									input_tip.setAttribute("disabled", "disabled");
								}else{
									input_tip.removeAttribute("disabled");
								}
			...
			$("#act_act_padre input[type=radio]").click(function(){

				var id = this.id;

				if( id == "act_d" || id == "act_b" ){
					ocultar_tipo(1);
					rollback();
				}else{
					if( id == "act_c" || id == "act_a" ){
						ocultar_tipo(0);
					}
				}
				...
				function ocultar_tipo(tipo){

					var sect_ubi = $("#padre_ubicacion_laboral");
					var tipo_input = $("#tipo_t");

					if(tipo){
						sect_ubi.css({display:"none"});
						tipo_input[0].value = 1;

					}else{
						sect_ubi.css({display:"block"});
						tipo_input[0].value = 0;
					...
					function rollback(){
						...
						var input = $("#padre_ubicacion_laboral input");
						var input_info = null;
						...
						for(var x = 0; x < input.length; x++){
							...
							if( input_info.type == "radio" ){
								input_info.checked = false;
							}else{
								if( input_info.type == "text" || input_info.type == "hidden"){
									input_info.value = "";
								}
							}
